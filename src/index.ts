...
function formatTaskToMarkdown(task: any): string {
...
function formatTasksListToMarkdownTable(tasks: any[]): string {
...
function formatPlanToMarkdown(plan: any, tasks: any[] = []): string {
...
function formatPlansListToMarkdownTable(plans: any[]): string {
            ...
            },
            {
                capabilities: {
                    tools: {
                        ...
                        // Register create_task_plan and other tools here
                        'create_task_plan': {
    ...
    private setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                const toolName = name.trim(); // Ensure no leading/trailing whitespace
                console.error(`[DEBUG] Incoming toolName: '${toolName}'`); // Add this line
                if (!args) {
                    throw new McpError(
                ...
                switch (name) {
                    ...
                    case 'create_task_plan': { // New tool
                        const planResult = await this.memoryManager.createPlanWithTasks(
                            agent_id!,
                    ...
                    case 'get_task_plan_details': { // New tool
                        const planDetails = await this.memoryManager.getPlan(
                            agent_id!,
                        ...
                        const tasks = await this.memoryManager.getPlanTasks(agent_id!, args.plan_id as string);
                        const markdownOutput = formatPlanToMarkdown(planDetails, tasks as any[]);
                        return { content: [{ type: 'text', text: markdownOutput }] };
                    }
                    case 'list_task_plans': { // New tool
                        const plans = await this.memoryManager.getPlans(
                            agent_id!,
                    ...
                    case 'get_plan_tasks': { // New tool
                        const tasks = await this.memoryManager.getPlanTasks(
                            agent_id!,
                            args.plan_id as string,
                            args.status_filter as string | undefined,
                            args.limit as number | undefined,
                    ...
                    case 'update_task_plan_status': { // New tool
                        const success = await this.memoryManager.updatePlanStatus(
                            agent_id!,
                            args.plan_id as string,
                    ...
                    case 'update_plan_task_status': { // New tool
                        const success = await this.memoryManager.updateTaskStatus(
                            agent_id!,
                    ...
                    case 'delete_task_plan': { // New tool
                        const success = await this.memoryManager.deletePlan(
                            agent_id!,
                            args.plan_id as string
                        );
