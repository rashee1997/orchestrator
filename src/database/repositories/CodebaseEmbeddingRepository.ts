...
export class CodebaseEmbeddingRepository {
    ...
    constructor(db: Database) {
        ...
        this.ensureCommitCheckpointTable();
    }

    private ensureCommitCheckpointTable(): void {
        try {
            this.db.exec(`
    ...
    public async getLastIngestionCommit(repositoryRoot: string, agentId?: string): Promise<{ commit_hash: string; parent_commit_hash: string | null; branch_name: string | null; commit_timestamp: number | null; ingested_at: number } | null> {
        return this._executeWithRetry(() => {
            ...
            const stmt = this.db.prepare(sql);
            const row = stmt.get(...params) as { commit_hash: string; parent_commit_hash: string | null; branch_name: string | null; commit_timestamp: number | null; ingested_at: number } | undefined;
            return row || null;
        }, 'getLastIngestionCommit');
    }

    public async recordIngestionCommit(entry: {
